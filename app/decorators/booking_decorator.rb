class BookingDecorator < Draper::Decorator
  ...
  def deposit_status 
    action = object&.requests&.deposit&.last&.action
    action = Request.actions['not_requested'] if action.nil? 

    h.content_tag(:span, action.upcase, class: 'badge text-bg-warning deposit-tag', style: 'cursor:pointer', data: { type: 'deposit'} )
  end

  def id_status 
    action = object&.requests&.id&.last&.action
    action = Request.actions['not_requested'] if action.nil? 
  ...
  def terms_status 
    action = object&.requests&.terms&.last&.action
    action = Request.actions['not_requested'] if action.nil? 
